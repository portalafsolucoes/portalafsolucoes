...
import StandardPlanDetailPanel from '@/components/standard-plans/StandardPlanDetailPanel'
...
export default function StandardMaintenancePlanPage() {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      ...
      const anyOpen = Object.entries(stepDropdownOpen).some(([k, v]) => v)
      if (!anyOpen) return
  ...
  // Recarregar sequência quando os campos chave mudam (apenas na criação)
  useEffect(() => {
    if (showFormModal && !editingId) {
      fetchNextSequence(formData.familyId || '', formData.familyModelId || '', formData.serviceTypeId || '')
    }
  }, [showFormModal, editingId, formData.familyId, formData.familyModelId, formData.serviceTypeId, fetchNextSequence])
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Salvar tarefas (apenas se houver tarefas preenchidas)
      ...
      if (validTasks.length > 0) {
        ...
        // Na edição, se não há tarefas válidas, limpar tarefas existentes
  ...
  return (
    <PageContainer variant="full" className="overflow-hidden p-0">
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-card divide-y divide-gray-200">
                  {filteredStandard.length === 0 ? (
                    <tr><td colSpan={9} className="px-6 py-8 text-center text-muted-foreground">Nenhum plano padrão cadastrado.</td></tr>
      ...
      {/* Modal de criação/edição */}
      <Modal isOpen={showFormModal} onClose={() => setShowFormModal(false)} title={editingId ? 'Editar Plano Padrão' : 'Novo Plano Padrão'}>

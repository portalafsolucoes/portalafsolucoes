  ...
  return (
    ...
    >
      <div className="h-60">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={data} margin={{ left: 0, right: 16, top: 8, bottom: 8 }}>
            ...
            <YAxis tick={{ fontSize: 11, fill: CHART_COLORS.muted }} domain={[0, 100]} tickFormatter={v => `${v}%`} />
            <Tooltip
            ...
            />
            {/* Meta PMC: linha tracejada cinza claro */}
            <ReferenceLine y={90} stroke={CHART_COLORS.mutedLight} strokeDasharray="4 4" strokeWidth={1.5} />
            {/* PMC: area preta com gradiente */}
            <Area type="monotone" dataKey="pmc" stroke={CHART_COLORS.primary} strokeWidth={2} fill="url(#pmcFill)" />
            {/* Disponibilidade: linha tracejada cinza medio para distinguir do PMC */}
            <Line type="monotone" dataKey="availability" stroke={CHART_COLORS.muted} strokeWidth={2} strokeDasharray="6 3" dot={{ r: 3, fill: CHART_COLORS.muted }} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
      <div className="flex items-center gap-4 text-[11px] mt-2 flex-wrap">
        ...
        <span className="flex items-center gap-1.5">
          <span className="h-0.5 w-4 border-t border-dashed" style={{ borderColor: CHART_COLORS.muted }} />
          <span className="text-muted-foreground">Disponibilidade</span>
        </span>
        <span className="flex items-center gap-1.5">
          <span className="h-0.5 w-4 border-t border-dashed" style={{ borderColor: CHART_COLORS.mutedLight }} />

...
- Cada tarefa da OS pode ter **mao de obra ou especialidade dedicada**: o planejador atribui um ou mais manutentores (papel canonico `MANUTENTOR`) ou cargos profissionais responsaveis por aquela tarefa especifica, persistidos em `TaskResource`. A definicao so esta disponivel nos mesmos cenarios da janela planejada — OS individual manual em `/work-orders` e OS originada de plano `Periodo = Unica` (criacao e edicao). Em OS de plano `Periodo = Repetitiva` o bloco fica oculto na UI e o servidor rejeita o payload. Materiais e ferramentas continuam consolidados a nivel OS (sem vinculo por tarefa); apenas mao de obra e especialidade podem ser atribuidas por tarefa
- O modelo de impressao da OS segue o layout oficial: cabecalho compacto com logo + Nº do Plano (quando existir) + Nº da OS, dois campos de `Data e Hora Inicio` / `Data e Hora Fim` para preenchimento manual, blocos `Ativo e Localizacao`, `Resumo da O.S.` (5 colunas: Status, Prioridade, Tipo de Servico, Criado em, Vencimento), `Descricao` em quadro proprio com quebras de linha preservadas, `Tarefas` com cabecalho `Inicio | Fim | Manutentor/Especialidade | Duracao` por tarefa e etapas com checkbox + linha de resposta, `Recursos - Materiais` e `Recursos - Ferramentas` em **2 colunas lado a lado** com linhas em branco extras, `Observacoes` com area livre e **uma unica assinatura** (Executante). A folha A4 suporta paginacao multi-pagina automatica — conteudo grande quebra em quantas folhas A4 forem necessarias sem perder informacao

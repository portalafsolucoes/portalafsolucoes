...
- `TaskResource` (FK em `Task`, `onDelete: Cascade`) armazena **recursos por tarefa**: `LABOR`, `SPECIALTY`, `MATERIAL` e `TOOL`. Campos: `id`, `resourceType` (`LABOR | SPECIALTY | MATERIAL | TOOL`), `hours: Decimal?(10,2)`, `quantity: Float?`, `unit: String?`, `userId?` (FK `User`, `ON DELETE SET NULL`), `jobTitleId?` (FK `JobTitle`, `ON DELETE SET NULL`), `resourceId?` (FK `Resource`, `ON DELETE SET NULL`). Indices em `taskId`, `userId`, `jobTitleId`, `resourceId`. Migrations canonicas: `prisma/migrations/20260502120000_add_task_resource/migration.sql` (criacao c/ LABOR/SPECIALTY) + `extend_task_resource_with_material_and_tool` (adiciona `resourceId`, `unit` e FK p/ Resource para suportar MATERIAL/TOOL — fiel ao MODELO DE OS_REV1.pdf que tem RECURSOS TAREFA N)
- `TaskResource` so e usado em **OSs manuais (sem `assetMaintenancePlanId`) ou OSs originadas de plano UNICA** (`AssetMaintenancePlan.period = 'UNICA'`). Planos (`StandardMaintenanceTask`/`AssetMaintenanceTask`) **nao** recebem este conceito — o gating vive no servidor (`isTaskResourcesAllowed`) e bloqueia POST/PATCH com `tasks[].resources[]` quando o plano e `REPETITIVA`. `WorkOrderResource` (a nivel OS) permanece como **camada de compat** para OSs legadas; OSs novas usam exclusivamente `TaskResource` para os 4 tipos. O print da OS (`WorkOrderPrintView`) renderiza recursos por tarefa e usa `WorkOrderResource` apenas como fallback visual quando nenhuma tarefa tem MATERIAL/TOOL atribuidos

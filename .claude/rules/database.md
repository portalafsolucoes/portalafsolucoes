...
- `TaskResource` (FK em `Task`, `onDelete: Cascade`) armazena **mao de obra (`LABOR`) e especialidade (`SPECIALTY`) por tarefa**. Campos: `id`, `resourceType` (`LABOR | SPECIALTY`), `hours: Decimal?(10,2)`, `quantity: Float?`, `userId?` (FK `User`, `ON DELETE SET NULL`), `jobTitleId?` (FK `JobTitle`, `ON DELETE SET NULL`). Indices em `taskId`, `userId`, `jobTitleId`. Migration canonica: `prisma/migrations/20260502120000_add_task_resource/migration.sql`
- `TaskResource` so e usado em **OSs manuais (sem `assetMaintenancePlanId`) ou OSs originadas de plano UNICA** (`AssetMaintenancePlan.period = 'UNICA'`). Planos (`StandardMaintenanceTask`/`AssetMaintenanceTask`) **nao** recebem este conceito — o gating vive no servidor (`isTaskResourcesAllowed`) e bloqueia POST/PATCH com `tasks[].resources[]` quando o plano e `REPETITIVA`. LABOR/SPECIALTY podem coexistir em `WorkOrderResource` (a nivel OS) por compat com OSs antigas, mas a impressao oficial busca mao de obra apenas em `TaskResource`
- `WorkOrderResource.taskId` (FK opcional para `Task`, `ON DELETE SET NULL`) vincula um material ou ferramenta a uma tarefa especifica da OS. Suporta o bloco "RECURSOS TAREFA N" do print fiel ao `MODELO DE OS_REV1.pdf`. Linhas legadas com `taskId = NULL` (criadas antes da migration `20260506180000_add_task_id_to_wo_resource`) caem no bloco da primeira tarefa por fallback no print. LABOR/SPECIALTY a nivel OS aceitam `taskId`, mas o print ignora — manutentores e especialidades sao lidos exclusivamente de `TaskResource`. Indice em `taskId`

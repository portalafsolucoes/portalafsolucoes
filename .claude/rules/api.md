...
## Recursos por Tarefa (`tasks[].resources[]`)
- `POST /api/work-orders` e `PATCH /api/work-orders/[id]` aceitam um array opcional `resources` por tarefa, com items `{ resourceType: 'LABOR' | 'SPECIALTY' | 'MATERIAL' | 'TOOL', userId?, jobTitleId?, resourceId?, hours?, quantity?, unit? }`. Helpers canonicos em `src/lib/workOrders/taskResources.ts`: `isTaskResourcesAllowed`, `validateTaskResources`, `validateLaborUsers`, `insertTaskResources`
...
- `LABOR` exige `userId` e `userId` precisa ter papel canonico `MANUTENTOR` na empresa ativa (mesma matriz de `assignedToId`); falha com `400` caso contrario. `SPECIALTY` exige `jobTitleId`. `MATERIAL` e `TOOL` exigem `resourceId` (FK para o cadastro `Resource`); `MATERIAL` aceita ainda `unit` opcional alem da unidade do proprio `Resource`. O print da OS renderiza Materiais com `Qtd | Unidade | Nome` e Ferramentas com `Qtd | Nome` (modelo nao tem coluna unidade em ferramentas)
- `GET /api/work-orders/[id]` e `?ids=` retornam `tasks(*, resources:TaskResource(id, resourceType, hours, quantity, unit, user:User(id, firstName, lastName), jobTitle:JobTitle(id, name), resource:Resource(id, name, unit)))`. O modo `summary=true` nao retorna tasks e segue inalterado
...
- `WorkOrderResource` (a nivel OS) **nao** e mais a fonte primaria de Materiais/Ferramentas em OSs novas — virou camada de compat para OSs legadas. O print da OS so usa `WorkOrderResource` como fallback visual quando nenhuma tarefa tem MATERIAL/TOOL atribuidos
...
## Inspecao de Area (`/api/inspections`)
- Feature `area-inspections` no `API_MODULE_MAP`. Acesso: `SUPER_ADMIN`, `ADMIN`, `PLANEJADOR` com matriz `{ view, create, edit, delete }`. `MANUTENTOR` recebe `{ view, edit }` (sem `create`/`delete`) e so opera inspecoes onde `assignedToId = session.id` (filtro automatico em `GET /api/inspections` e em `GET /api/inspections/[id]`)
- Workflow oficial: `RASCUNHO -> EM_REVISAO -> FINALIZADO`. Status virtual `ATRASADO` e derivado pelo servidor quando `(status === 'RASCUNHO' OR status === 'EM_REVISAO') AND today > dueDate + 1 dia`; nao e persistido
- `GET /api/inspections` lista do tenant + unidade ativa, ordenada por `createdAt DESC`. Aceita `?status=`, `?assignedToId=`, `?unitId=`. Inclui `requestCount` (SSs com `inspectionId = inspection.id`) e `isOverdue` por item. `MANUTENTOR` ve apenas inspecoes atribuidas a ele
- `GET /api/inspections/[id]` retorna detalhe completo com `assets[].steps[]` ordenados, `assignedTo`, `submittedForReviewBy`, `finalizedBy`, `reopenedBy`, `createdBy`, `standardChecklist`, `request` por step (quando ja gerou SS) e `isOverdue`. `MANUTENTOR` so acessa quando `assignedToId === session.id`. Cross-unit retorna `403`
- `POST /api/inspections` cria a partir de `{ standardChecklistId, description, dueDate, assignedToId }`. Valida que o checklist pertence a empresa e a unidade ativa; valida `assignedToId` na empresa; busca bens compativeis (`Asset.workCenterId === checklist.workCenterId AND Asset.unitId === session.unitId AND archived === false AND (familyId, familyModelId) IN groups`); retorna `400` quando nenhum bem compativel; faz **snapshot** dos bens + etapas (clona `name`, `protheusCode`, `optionType` do `GenericStep`) em transacao com rollback manual via `DELETE inspection` em caso de erro. Numero gerado por `generateInspectionNumber` (6 digitos, sequencia por `(companyId, unitId)`); UNIQUE constraint protege contra colisao com retry uma vez
- `PUT /api/inspections/[id]` aceita `{ description?, dueDate?, assignedToId? }` apenas em RASCUNHO/EM_REVISAO. Apenas gestores
- `PUT /api/inspections/[id]/answers` recebe `{ answers: [{ stepId, answer: 'OK'|'NOK'|'NA'|null, notes?: string|null }] }`. Em RASCUNHO: manutentor atribuido OU gestor; em EM_REVISAO: somente gestor; FINALIZADO: bloqueado (`409`). Escopo dos `stepId` e validado server-side (steps de outras inspecoes sao silenciosamente ignorados). Setar `answer = null` limpa `answeredById` e `answeredAt`
- `POST /api/inspections/[id]/submit-for-review` move RASCUNHO -> EM_REVISAO. Valida `count(steps WHERE answer IS NULL) === 0`; em caso contrario retorna `409` com `pending` na resposta. Manutentor atribuido OU gestor
- `POST /api/inspections/[id]/return-to-draft` move EM_REVISAO -> RASCUNHO. Limpa `submittedForReviewAt` e `submittedForReviewById`. Apenas gestores
- `POST /api/inspections/[id]/finalize` move EM_REVISAO -> FINALIZADO. Body `{ nokDetails: [{ stepId, title, description?, priority?, maintenanceAreaId, dueDate? }] }`. Valida que toda etapa com `answer === 'NOK' AND requestId IS NULL` tem `nokDetail` com `title` e `maintenanceAreaId`; valida que `maintenanceAreaId` pertence a empresa. Cria `Request` com `inspectionId`, `unitId = session.unitId`, `companyId`, `priority` (default `NORMAL`), status `PENDING`, e popula `requestNumber` via padrao `SS-XXXXXX` global. Vincula `AreaInspectionStep.requestId`. Em caso de erro de qualquer SS, **rollback** das criadas no batch. Apenas gestores
- `POST /api/inspections/[id]/reopen` move FINALIZADO -> EM_REVISAO. Preenche `reopenedAt` e `reopenedById`. Apenas gestores. SSs ja criadas permanecem intactas (opcao A — congelar)
- `DELETE /api/inspections/[id]` faz hard delete em RASCUNHO/EM_REVISAO; bloqueia FINALIZADO (`409`). Cascade SQL apaga `AreaInspectionAsset` e `AreaInspectionStep`
- `GET /api/maintenance-plans/standard-checklists/[id]/eligible-assets` lista bens compativeis na unidade ativa para preview do form de criacao. Mesma regra de compatibilidade do `POST /api/inspections`. Retorna `403` quando o checklist e de outra unidade

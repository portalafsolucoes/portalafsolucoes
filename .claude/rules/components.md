...
## Recursos por Tarefa em OS (OBRIGATORIO)
- O `WorkOrderFormModal` expoe, dentro do card de cada tarefa, um bloco `Recursos da Tarefa` usando o componente `<ResourceSelector>` com `allowedTypes={['LABOR', 'SPECIALTY', 'MATERIAL', 'TOOL']}` e `label="Recursos da Tarefa (Mao de Obra / Especialidade / Material / Ferramenta)"`. Cada item tem `userId` (LABOR — apenas usuarios com papel canonico `MANUTENTOR`), `jobTitleId` (SPECIALTY), ou `resourceId` + `unit?` (MATERIAL/TOOL — referencia ao cadastro `Resource`)
...
- O `WorkOrderDetailModal` exibe, dentro de cada card de tarefa, uma faixa cinza com badges `Manutentor: <nome>` ou `Especialidade: <cargo>` sempre que `task.resources` tiver itens. Materiais e Ferramentas por tarefa aparecem no print mas nao no detail panel atualmente
- O `WorkOrderPrintView` (e por extensao o `WorkOrdersBatchPrintView`) renderiza dentro de cada card de tarefa: (a) header com 4 colunas `Nome do Manutentor ou Especialidade | Duracao Tarefa | Data e Hora Inicio | Data e Hora Fim`; (b) tabela `Etapas/Resposta` com etapas em italico; (c) bloco condicional `RECURSOS TAREFA N` com Materiais (`Qtd | Unidade | Nome`) a esquerda e Ferramentas (`Qtd | Nome` — sem coluna unidade conforme MODELO DE OS_REV1.pdf) a direita. O bloco so aparece quando a tarefa tem MATERIAL ou TOOL alocado. `WorkOrderResource` (recursos a nivel OS) e usado APENAS como fallback visual no rodape quando NENHUMA tarefa tem MATERIAL/TOOL — para suportar OSs legadas
